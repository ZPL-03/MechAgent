...
CAD 导入和几何修复属于后续扩展能力。可执行路径为新增 CAD 工具适配器，使用 OpenCASCADE 形体导入、几何修复、特征识别、简化与局部网格尺寸场生成，将外部 CAD 形体转换为可求解的 `ModelParams`、网格策略和边界/载荷区域映射。该能力进入发布边界前需要提供 CAD 样例、几何修复失败诊断、网格质量证据、真实求解结果和 Studio 可视化验收。
