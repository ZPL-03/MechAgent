...
系统接收自然语言工程需求，将需求转化为可执行、可审计、可复现的仿真任务链路，并在 Studio、CLI 和 SDK 中输出统一的工程结果。当前发布边界聚焦结构线弹性静力分析，覆盖梁、矩形板、单孔/偏心孔/多孔/槽孔薄板和矩形实体块的自然语言输入、参数建模、网格划分、真实求解、后处理、校核、3D 可视化和 Markdown 报告。TC-01 至 TC-05 承担回归校准，开孔薄板承担复杂几何前处理、布尔建模、局部网格控制和自由开孔边界展示。CAD 基础链路支持 STEP/IGES/BREP 形体摘要、几何候选和自然语言补全到结构静力参数；通用几何修复、特征识别和自动区域语义映射按成熟度门禁推进。后续分析类型、几何族、求解器和网格器通过能力注册与工具注册进入同一套框架。
...
7. 查看主结果、参考值、相对误差、验收结论、Agent 链路摘要和阶段产物路径。
8. 阅读、复制和下载自动生成的 Markdown 工程报告。
9. 复制或下载结构化摘要 JSON，用于外部系统复核、归档或复跑。
10. 回看本地运行历史中的请求、摘要和报告，并重跑任一历史请求恢复 3D 结果。
...
界面设计以工程判读为优先级。布局稳定，信息密度适合反复使用；关键指标不因中文短词换行破坏阅读；按钮文本不溢出；结果视口是核心区域；左侧预检区用于快速判断请求是否可执行，下方检查区用于快速复核验收状态、阶段事件、Agent 产物和链路状态。复合请求在 UI 中以预检摘要、任务矩阵和任务标签呈现，每个任务保持独立的几何、网格、结果、错误记录和报告摘要。
...
Studio 的 Agent 链路面板只消费公开摘要，显示每个 Agent 的结构化产物摘要、网格质量诊断、结果状态和脱敏 LLM trace 状态。原始 LLM prompt、response、密钥和内部调试上下文不进入前端显示。
...
| 等级 | 定义 | 进入条件 |
| --- | --- | --- |
| 研究能力 | Agent 能识别工程意图，并形成候选参数、网格策略或工具调用方案 | schema 草案、失败诊断、示例请求 |
| 可执行能力 | 能从自然语言请求生成求解输入，并完成真实求解和后处理 | 端到端求解记录、阶段产物、报告输出 |
| 可验证能力 | 有解析参考、工程基准、网格收敛或跨工具对比证据 | 误差阈值、自动化测试、质量门禁 |
| 发布能力 | Studio、CLI、SDK、文档、清理策略和安装流程一致 | 前端构建、包构建、文档构建、完整门禁 |

当前 `0.1.0` 发布能力处于“发布能力”等级，范围限定为结构线弹性静力分析。CAD 基础能力覆盖形体摘要、包围盒几何候选和自然语言补全到结构静力参数；几何修复、特征识别、装配体、自动区域语义映射、非线性材料、接触、渐进损伤和多物理场进入发布范围前需要达到同等质量门槛。
...
- 板：使用单元面、边界支承、载荷箭头、位移标量场和应力标量场；开孔薄板在几何模式显示单孔、多孔或槽孔参数化开孔轮廓，在网格和结果模式读取 `.inp` 单元面表达开孔自由边界。梁网格以矩形截面分段棱柱表达 B31 单元拓扑，节点点标记显示单元连接位置。结果模式默认显示 `U` 位移模量，场量下拉菜单可切换 `Ux`、`Uy`、`Uz` 位移分量；`.frd` 存在应力场时可切换 `S Mises`、`Sxx`、`Syy`、`Szz`、`Sxy`、`Syz` 和 `Sxz`。边界条件和载荷从 `ModelParams` 进入可视化场景，几何模式显示固定端夹持、边界支承、集中力、线载荷、均布压力和端面载荷符号，结果模式只显示变形、网格边、结果场和颜色图例；壳单元 `.frd` 派生节点场按真实网格节点顺序对齐。
...
- 开孔薄板：矩形薄板单孔、偏心孔、多孔或水平长圆槽孔，Gmsh 布尔建模，外边界简支，开孔边界自由，纯全局 Z 向均布压力，主结果为最大位移。
...
当前复杂几何能力以参数化模型为主：梁、矩形板、单孔/偏心孔/多孔/槽孔薄板和矩形实体块都由 `ModelParams` 驱动，开孔薄板通过 Gmsh OCC 布尔建模生成开孔边界和局部网格。Gmsh 网格路径提取 `min_sicn` 与 `mean_sicn` 质量指标，MeshAgent 按 `mesher.min_quality` 对 `min_*` 指标做质量门禁，Studio 在 Agent 链路中展示网格数量、质量摘要和失败诊断。

CAD 基础链路通过 CAD 内核适配器读取 STEP/IGES/BREP，形成 `CADGeometrySummary` 与 `GeometryCandidate`，再由自然语言补全材料、载荷和边界，转换为结构静力 `ModelParams`。该链路适用于可由包围盒和薄度/细长比可靠归类的基础形体。通用几何修复、去特征、孔槽/圆角/倒角等特征识别、装配体关系、自动边界/载荷区域语义映射和局部尺寸场生成仍属于复杂前处理扩展范围，进入发布边界前需要提供 CAD 样例、失败诊断、网格质量证据、真实求解结果和 Studio 可视化验收。
...
路线图遵循“先形成可信闭环，再扩展物理域和工具生态”的顺序。完整的长周期阶段、工具生态接入、验证体系、里程碑与治理见 [长周期开发计划书](development_plan.md)。

| 阶段 | 能力目标 | 验收证据 |
| --- | --- | --- |
| 0.1 结构静力闭环 | 梁、矩形板、单孔/偏心孔/多孔/槽孔薄板、矩形实体块的自然语言到真实求解、3D 可视化和报告 | TC-01 至 TC-05、二十八个自然语言案例、LLM smoke、Studio 浏览器验证 |
| 0.2 复杂前处理泛化 | 加强筋板、支架类二维轮廓、CAD 几何修复、特征识别、更多边界和载荷区域、参数扫描和批量任务 | CAD/参数化工程案例、网格质量诊断、网格收敛证据、UI 多任务验收 |
| 0.3 高级结构分析 | 几何非线性、材料非线性、接触、渐进损伤、屈曲和模态分析 | 解析参考、工程基准、跨工具对比、非线性收敛诊断和报告字段 |
| 0.4 多物理场与动态过程 | 热分析、热-结构耦合、瞬态动力学和场量映射 | 多场量后处理、守恒量检查、真实求解案例和可视化验收 |
| 0.5 工具生态 | 更多开源求解器、网格器、后处理器、材料库和知识源接入 | 工具注册测试、跨工具结果一致性、安装验证 |
| 1.0 发布级产品 | 插件包、公开 API 文档、任务队列、远程工作站执行、结果归档和审计追踪 | wheel 安装验证、端到端 UI 验收、文档站点、发布门禁和归档可复现记录 |
