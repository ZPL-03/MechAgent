...
当前复杂几何能力以参数化模型为主：梁、矩形板、单孔/偏心孔/多孔薄板和矩形实体块都由 `ModelParams` 驱动，开孔薄板通过 Gmsh OCC 布尔建模生成孔洞边界和局部网格。Gmsh 网格路径提取 `min_sicn` 与 `mean_sicn` 质量指标，MeshAgent 按 `mesher.min_quality` 对 `min_*` 指标做质量门禁，Studio 在 Agent 链路中展示网格数量、质量摘要和失败诊断。
