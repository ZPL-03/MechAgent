...
系统接收自然语言工程需求，将需求转化为可执行、可审计、可复现的仿真任务链路，并在 Studio、CLI 和 SDK 中输出统一的工程结果。当前发布边界聚焦结构线弹性静力分析，覆盖梁、矩形板、单孔/偏心孔/多孔/槽孔薄板和矩形实体块的自然语言输入、参数建模、网格划分、真实求解、后处理、校核、3D 可视化和 Markdown 报告。TC-01 至 TC-05 承担回归校准，开孔薄板承担复杂几何前处理、布尔建模、局部网格控制和自由开孔边界展示。后续分析类型、几何族、求解器和网格器通过能力注册与工具注册进入同一套框架。
...
当前 `0.1.0` 发布能力处于“发布能力”等级，范围限定为结构线弹性静力分析。CAD 导入、几何修复、非线性材料、接触、渐进损伤和多物理场属于后续能力，进入发布范围前需要达到同等质量门槛。
...
CAD 导入和几何修复属于后续扩展能力。可执行路径为 CAD 工具适配器使用 OpenCASCADE 形体导入、几何修复、特征识别、简化与局部网格尺寸场生成，将外部 CAD 形体转换为可求解的 `ModelParams`、网格策略和边界/载荷区域映射。该能力进入发布边界前需要提供 CAD 样例、几何修复失败诊断、网格质量证据、真实求解结果和 Studio 可视化验收。
...
| 0.2 复杂前处理泛化 | 多孔板、偏心孔、加强筋板、支架类二维轮廓、CAD 导入、几何修复、更多边界和载荷区域、参数扫描和批量任务 | CAD/参数化工程案例、网格质量诊断、网格收敛证据、UI 多任务验收 |
