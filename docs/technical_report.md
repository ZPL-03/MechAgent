...
- 检查区：作业状态、验收状态、求解流程、阶段产物类型标签、路径复制入口、摘要 JSON 复制和摘要 JSON 下载。顶栏运行环境胶囊显示必需诊断项通过数，可通过悬停查看配置路径、可选项状态和失败项名称。宽屏桌面视口位于右侧并使用整列滚动，验收状态面板按内容自适应高度；中等宽度视口位于主工作区下方，移动端单列展示。
...
Studio 不改变 Agent 通信契约。浏览器消费 `/api/health` 返回的 Python 执行器生成 CLI 复现命令，消费 `/api/diagnostics` 返回的本地运行环境摘要显示 Python、配置、依赖、静态资源、注册表、求解器、LLM 配置、前端工具和 Git 状态，消费 `/api/jobs` 返回的作业状态、阶段事件、公开摘要和可视化列表；
