...
- 检查区：作业状态、验收状态、求解流程、阶段产物类型标签、路径复制入口、摘要 JSON 复制和摘要 JSON 下载。顶栏运行环境面板显示必需诊断项通过数、可选项状态、配置路径和逐项诊断摘要，键盘和鼠标均可展开查看。宽屏桌面视口位于右侧并使用整列滚动，验收状态面板按内容自适应高度；中等宽度视口位于主工作区下方，移动端单列展示。

求解流程列表由当前运行摘要、错误记录和编排阶段事件驱动，工作流状态直接来源于后端，不由前端合成。LangGraph 与顺序工作流在 Planner、Designer、MeshAgent、SolverAgent、PostProcAgent、AnalystAgent 和 ReporterAgent 开始、完成或失败时发送进度事件。Studio 创建后端作业后轮询
...
Studio 不改变 Agent 通信契约。浏览器消费 `/api/health` 返回的 Python 执行器生成 CLI 复现命令，消费 `/api/diagnostics` 返回的本地运行环境摘要，并在顶栏运行环境面板展示 Python、配置、依赖、静态资源、注册表、求解器、LLM 配置、前端工具和 Git 状态，消费 `/api/jobs` 返回的作业状态、阶段事件、公开摘要和可视化列表；
...
动画约束、触控行为、长文本换行、本地化数值格式和浏览器本地运行历史。运行历史保留自然语言请求、紧凑摘要和限长 Markdown 报告文本；选中历史项后可重跑此请求恢复几何、网格和结果视图，3D 场景本身不写入浏览器存储，避免大体积节点场占用 `localStorage`。宽屏桌面视口展示左侧请求区、中央结果区和右侧检查区三栏布局；宽度不超过 1800px 时检查区位于主工作区下方并保持两列排布，宽度不超过 900px 时切换为单列布局。初始态、运行态和结果态保持固定响应式断点和面板尺寸约束，避免 3D 画布、报告或检查区造成浏览器横向溢出。结果视口、报告面板、验收面板、流程面板和阶段产物面板均有响应式约束，宽屏右侧检查区采用整列滚动，验收面板不使用内部固定高度裁切；运行前后保持左右外边距一致，避免 3D 画布、报告和流程面板产生横向溢出或异常拉伸。README 展示图
