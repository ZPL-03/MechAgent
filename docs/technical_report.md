...
- 检查区：作业状态、验收状态、求解流程、阶段产物类型标签、路径复制入口、摘要 JSON 复制和摘要 JSON 下载。宽屏桌面视口位于右侧并使用整列滚动，验收状态面板按内容自适应高度；中等宽度视口位于主工作区下方，移动端单列展示。
...
动画约束、触控行为、长文本换行、本地化数值格式和浏览器本地运行历史。宽屏桌面视口展示左侧请求区、中央结果区和右侧检查区三栏布局；宽度不超过 1800px 时检查区位于主工作区下方并保持两列排布，宽度不超过 900px 时切换为单列布局。初始态、运行态和结果态保持固定响应式断点和面板尺寸约束，避免 3D 画布、报告或检查区造成浏览器横向溢出。结果视口、报告面板、验收面板、流程面板和阶段产物面板均有响应式约束，宽屏右侧检查区采用整列滚动，验收面板不使用内部固定高度裁切；运行前后保持左右外边距一致，避免 3D 画布、报告和流程面板产生横向溢出或异常拉伸。README 展示图
...
| ReporterAgent | `TaskRunRecord` 列表 | Markdown 报告、LLM 工程解释和 `reporter_llm_trace` |
...
ReporterAgent 在启用 LLM Agent 时把求解摘要、后处理标量、网格元数据、载荷和边界条件发送给 LLM，递归剥离执行链路 trace 审计字段后输出“LLM 工程解释”章节，解释结果含义、可信度、局限和复核建议。
...
- LLM 结构化链路：SDK 单次 LLM trace 开关、LLM HTTP 后端契约、LLM 端到端 smoke 脚本、
  ...
  LLM 多孔薄板孔洞归一化、MeshAgent LLM 网格策略校验、ReporterAgent LLM 工程解释报告、
...
- Studio 前端构建：Vite 构建通过；Markdown 渲染依赖拆分为独立 chunk，
  主入口 gzip 约 84 KB，3D 视口 chunk gzip 约 149 KB。
- `scripts/run_llm_smoke.py`：通过；Designer 结构化补参 trace 成功，Planner、MeshAgent、
  ...
  Markdown 报告包含基于 FEM 结果生成的“LLM 工程解释”章节。
