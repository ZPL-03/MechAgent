...
动画约束、触控行为、长文本换行、本地化数值格式和浏览器本地运行历史。运行历史保留自然语言请求、紧凑摘要和限长 Markdown 报告文本；3D 场景不写入浏览器存储，避免大体积节点场占用 `localStorage`。宽屏桌面视口展示左侧请求区、中央结果区和右侧检查区三栏布局；宽度不超过 1800px 时检查区位于主工作区下方并保持两列排布，宽度不超过 900px 时切换为单列布局。初始态、运行态和结果态保持固定响应式断点和面板尺寸约束，避免 3D 画布、报告或检查区造成浏览器横向溢出。结果视口、报告面板、验收面板、流程面板和阶段产物面板均有响应式约束，宽屏右侧检查区采用整列滚动，验收面板不使用内部固定高度裁切；运行前后保持左右外边距一致，避免 3D 画布、报告和流程面板产生横向溢出或异常拉伸。README 展示图
