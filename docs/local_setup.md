...
Studio 命令输出服务监听地址和浏览器入口。`--open-browser` 打开浏览器入口；监听地址为 `0.0.0.0` 或 `::` 时，浏览器入口使用 `127.0.0.1`。`--request`、`--llm-agents` 和 `--view geometry|mesh|result` 生成带自然语言请求、参数补全状态和初始视图的可复现工作台入口。`--auto-run` 与 `--request` 同时使用时，工作台页面加载后自动提交当前请求。
