...
| 复杂参数化前处理 | 已接入基础能力 | 开孔薄板支持单孔、偏心孔、多孔和水平长圆槽孔布尔建模；CAD 基础链路支持 STEP/IGES/BREP 形体摘要、几何候选和自然语言补全到结构静力参数 |
| CAD 几何修复与特征识别 | 路线规划 | 通用几何修复、去特征、装配体、自动边界/载荷区域语义映射进入发布边界前需要 CAD 样例、失败诊断、网格质量和真实求解证据 |
