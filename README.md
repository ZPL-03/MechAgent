...
Studio 默认监听 `127.0.0.1:8765`。界面包含自然语言请求、运行环境诊断、任务预检、参数补全开关、示例库、运行历史、作业状态、求解流程、Agent 链路、3D 结果视口、验收指标、阶段产物、摘要 JSON 和 Markdown 报告。工作台链接支持恢复请求、LLM 开关和视图模式；报告、摘要和复现命令均可复制或下载。
...
- 扩展网格策略：自适应网格、局部加密建议、质量诊断报告和多网格器接入。
