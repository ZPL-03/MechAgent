...
当前发布能力聚焦结构线弹性静力分析，内置 CalculiX 与 Gmsh 适配，支持梁、薄板、开孔薄板和实体块等参数化模型族。项目同时保留清晰的能力边界和扩展契约，便于继续接入新的几何类型、材料模型、求解器、网格器和后处理策略。
...
求解完成后，Studio 展示结果场、网格、验收摘要、阶段产物、结构化摘要和 Markdown 工程报告。
...
MechAgent 支持 Python 3.9 及以上版本。建议在独立虚拟环境中安装依赖。
...
Studio 默认监听 `127.0.0.1:8765`。界面包含自然语言请求、运行环境诊断、任务预检、参数补全开关、示例库、运行历史、作业状态、求解流程、3D 结果视口、验收指标、阶段产物、摘要 JSON 和 Markdown 报告。工作台链接支持恢复请求、LLM 开关和视图模式；报告、摘要和复现命令均可复制或下载。

## 可视化与报告

Studio 的可视化层服务于完整仿真工作流，而不是单个固定算例。几何视图用于检查参数化模型、边界条件和载荷施加；网格视图用于检查单元拓扑、节点、孔洞边界和局部网格质量；结果视图用于查看变形、位移场、可用应力场和场量范围。后处理数据来自求解摘要、网格文件和结果文件，前端只负责把结构化结果表达为可交互视图。
...
- 已验证能力：结构线弹性静力分析。
- 已接入工具：CalculiX 求解器、Gmsh 网格能力和本地结构化网格生成。
- 已覆盖模型族：梁、矩形薄板、开孔薄板和矩形实体块。
- 材料模型：各向同性线弹性材料，支持内置材料别名和显式 `E`、`nu` 参数。
- LLM 使用：默认关闭；启用后参与能力选择、参数补全、网格策略建议和工程解释。Planner 会合并能力缺参诊断；`SimulationIntent.missing_fields` 非空时 Designer 仍尝试由 LLM 生成 `ModelParams`。本地 parser 已生成验证通过的本地 `ModelParams` 时，后续链路使用本地参数，LLM 输出进入 trace 审计；能力声明 `model_case_ids` 时，`ModelParams.case_id` 必须属于声明集合。SDK 单次覆盖使用独立配置副本，不改变持久配置。
...
完整产品路线、扩展边界和技术细节见 [docs/product_blueprint.md](docs/product_blueprint.md) 与 [docs/technical_report.md](docs/technical_report.md)。

## 后续路线

- 扩展几何生成能力：更复杂的参数化零件、CAD 导入、特征识别和几何修复。
- 扩展网格策略：自适应网格、局部加密建议、质量诊断和多网格器接入。
- 扩展求解能力：几何非线性、材料非线性、接触、热-结构耦合、动力学和渐进损伤。
- 扩展后处理能力：多场量比较、路径/截面查询、结果包络、报告对比和工程判据库。
- 扩展协作形态：批量任务、远程工作站执行、作业队列、项目归档和可追溯审计。
...
当前测试集包含 408 个测试，覆盖公开 API、Studio 服务、编排、LLM、知识库、网格契约、求解失败归一化、LangGraph 状态契约、自然语言案例、运行前预检和真实 CalculiX 验证。完整测试范围、质量结果和清理策略见 [docs/technical_report.md](docs/technical_report.md)。

## 文档

- [docs/index.md](docs/index.md)：文档入口和技术细节索引。
...
- [docs/technical_report.md](docs/technical_report.md)：架构、Agent 通信、schema、LLM 契约、工具适配、验证证据和质量结果。
