...
本地完整验证环境使用 `D:/anaconda3/envs/GPT/python.exe`。CalculiX 可执行文件通过
`CALCULIX_CCX` 指向本机路径，不依赖系统 PATH。
...
D:/anaconda3/envs/GPT/python.exe -m mechagent.cli run "求解长1000mm、截面20mmx40mm、材料钢的悬臂梁，一端固支，端部向下1000N集中力的静力响应" --json
```
...
测试集包含 336 个测试，覆盖公开 API、编排、LLM、知识库、MeshAgent 输出契约、求解失败归一化、
