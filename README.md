...
<p align="center">
  <img src="docs/assets/mechagent-logo.png" alt="MechAgent logo" width="128" height="128">
</p>
...
Studio 采用引导式工作流（输入 → 预检 → 运行 → 结果）、明暗双主题、自适应三栏布局、验收摘要和标签式检查器。
...
Studio 默认监听 `127.0.0.1:8765`。界面以引导式工作流组织：顶栏（品牌、运行状态、运行环境诊断、明暗主题切换）、贯穿式进度（输入 → 预检 → 运行 → 结果）、左栏（自然语言请求、任务预检、参数补全开关、工况库、运行历史、模型摘要）、中区（3D 几何/网格/结果视口、任务结果矩阵、Markdown 报告）、右栏（验收状态、求解流程、Agent 链路、阶段产物、摘要 JSON 标签页）。布局在宽屏/笔记本/窄屏间自适应。工作台链接支持恢复请求、LLM 开关和视图模式；报告、摘要和复现命令均可复制或下载。
...
| 模块 | 职责 |
| --- | --- |
| `mechagent-core` | 数据模型、材料、求解器适配、网格器适配、CAD 内核、作业执行器、后处理和验证基准 |
...
| CAD 几何候选 | STEP/IGES/BREP 形体摘要、包围盒几何候选、自然语言补全到结构静力参数 | `CADGeometrySummary`、`GeometryCandidate`、`ModelParams` |
...
- CAD 支持范围为 STEP/IGES/BREP 形体摘要、包围盒几何候选和自然语言补全到结构静力参数；通用几何修复、特征识别、装配体、自动载荷/边界区域语义映射和任意 CAD 直接稳健求解不属于当前发布边界。
- 当前验证范围不包含非线性材料、渐进损伤、接触和多物理场。
...
| 方向 | 目标 | 验收方式 |
| --- | --- | --- |
| 复杂几何前处理 | 参数化零件库、CAD 特征识别、几何修复、自动区域语义映射和装配体建模 | CAD 样例、几何修复诊断、网格质量证据、真实求解结果 |
...
```text
packages/
  mechagent-core/   # 数据模型、材料、CAD、执行器、网格器、求解器、后处理和解析参考
