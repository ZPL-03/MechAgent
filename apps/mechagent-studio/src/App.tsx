...
type AgentChainTone = "neutral" | "ok" | "warn" | "bad";
...
export function App() {
  ...
  const agentChain = useMemo(
    () => buildAgentChain(result, selectedTask),
    [result, selectedTask]
  );
      ...
      <div className="studio-layout">
        ...
        <aside className={`right-rail ${result ? "has-result" : ""}`} aria-label="仿真检查器">
          ...
          <section className="panel agent-chain-panel">
            <PanelTitle icon={<Network size={18} />} title="Agent 链路" />
...
function meshCountLabel(mesh: NonNullable<TaskSummary["mesh_result"]>) {
  ...
  const elements = finiteNumberValue(mesh.element_count) ?? finiteNumberValue(metadata?.element_count);
...
function postSummaryDetails(postSummary: Record<string, unknown> | null): DisplayItem[] {
  ...
  const scalarItems = scalars
    ? Object.entries(scalars)
        .slice(0, 4)
        .map(([key, value]) => ({ label: key, value: formatUnknownNumber(value) }))
    : [];
  ...
  return Object.entries(postSummary)
    ...
    .slice(0, 4)
    .map(([key, value]) => ({ label: key, value: compactDisplayValue(value) }));
}
...
function agentTraceDetails(trace: AgentTrace | null | undefined): DisplayItem[] {
  if (!trace) {
...
function agentChainToneLabel(tone: AgentChainTone) {
  if (tone === "ok") {
    return "通过";
  }
  if (tone === "warn") {
...
function compactDisplayValue(value: unknown) {
  if (typeof value === "string" && value.trim()) {
    return value.trim();
  }
  if (typeof value === "number" && Number.isFinite(value)) {
