...
| 能力 | 状态 |
| --- | --- |
| 自然语言到仿真任务 | 支持中英文结构静力请求、工程单位解析、复合请求拆分和缺参诊断 |
| Agent 编排 | Planner、Designer、MeshAgent、SolverAgent、PostProcAgent、AnalystAgent、ReporterAgent 通过 Pydantic v2 模型通信 |
| 工具接入 | 通过能力注册表和工具工厂选择 CalculiX、Gmsh、CAD 内核、作业执行器或配置中的默认工具 |
| 求解闭环 | 梁、矩形薄板、圆孔/多孔/槽孔开孔薄板和矩形实体块的线弹性静力分析 |
| CAD 几何候选 | STEP/IGES/BREP 形体摘要、包围盒几何候选和自然语言补全到结构静力参数 |
| 可视化与报告 | Studio 展示几何、网格、结果场、Agent 链路、验收指标、阶段产物和 Markdown 报告 |
| LLM Agent | 通过 OpenAI 兼容接口可选启用，公开摘要保留脱敏 trace |
...
Studio 后端使用 FastAPI 与 Uvicorn，由 `python -m mechagent.cli studio` 启动；`python -m mechagent.cli demo` 从示例库读取 `SC-27` 作为默认展示工况，打开 Studio 并自动提交长圆槽孔薄板静力请求，`--example` 可选择其他示例编号。
启动命令输出服务监听地址和浏览器入口；监听地址为 `0.0.0.0` 或 `::` 时，浏览器入口使用 `127.0.0.1`。
前端使用 React、TypeScript、Vite、Three.js 和 Markdown 渲染组件，构建后的静态资源位于
`mechagent/ui/static` 并随包发布。界面采用引导式工作流（输入 → 预检 → 运行 → 结果）与自适应三栏布局：宽屏为左侧输入、中部结果、右侧检查器；笔记本宽度检查器移至主工作区下方两列；窄屏切换为单列堆叠。顶栏提供明暗双主题切换（明 / 暗 / 跟随系统），右侧检查器固定展示验收状态，并以标签式详情面板组织求解流程、Agent 链路、阶段产物和摘要 JSON。界面展示自然语言请求、可展开运行环境面板、运行前预检、参数补全开关、示例库、当前工作台链接复制、基于当前 Python 执行器的 CLI 复现命令复制、运行历史、后端作业状态、求解流程、Agent 链路、验收指标、
