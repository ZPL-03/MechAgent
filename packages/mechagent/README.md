...
| 工具接入 | 通过能力注册表和工具工厂选择 CalculiX、Gmsh 或配置中的默认工具 |
...
`mechagent/ui/static` 并随包发布。界面采用引导式工作流（输入 → 预检 → 运行 → 结果）与自适应三栏布局：宽屏为左侧输入、中部结果、右侧检查器；笔记本宽度检查器移至主工作区下方两列；窄屏切换为单列堆叠。顶栏提供明暗双主题切换（明 / 暗 / 跟随系统），右侧检查器按可折叠分组组织验收状态、求解流程、Agent 链路、阶段产物和摘要 JSON。界面展示自然语言请求、可展开运行环境面板、运行前预检、参数补全开关、示例库、当前工作台链接复制、基于当前 Python 执行器的 CLI 复现命令复制、运行历史、后端作业状态、求解流程、Agent 链路、验收指标、
